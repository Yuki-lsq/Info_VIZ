...
#####Live music#####
################
music_venues <- read.csv("live-music-venues.csv")
...
#cafe
tourism_industry_data$IndustryColor <- 'orange'
...
# Setting markers colors for space type

music_venues$VenueTypeColor <- 'orange'

# Setting marker icons for different space types
music_venues$VenueTypeIcon[music_venues$space_type == "Hotel"] <- 'bed'
music_venues$VenueTypeIcon[music_venues$space_type == "Bar"] <- 'beer'
music_venues$VenueTypeIcon[music_venues$space_type == "Multi-use"] <- 'bank'
music_venues$VenueTypeIcon[music_venues$space_type == "Nightclub"] <- 'music'
music_venues$VenueTypeIcon[music_venues$space_type == "Performance space"] <- 'building'
music_venues$VenueTypeIcon[music_venues$space_type == "Theatre"] <- 'sign-language'
music_venues$VenueTypeIcon[music_venues$space_type == "Venue"] <- 'fort-awesome'
...
ui <- fluidPage(
  ...
  dashboardPage(
    skin ="yellow",
    dashboardHeader(title = "AUStourism"),
    dashboardSidebar(
      ...
      sidebarMenu(
        ...
        menuItem("Accommodation", tabName = "Accommodation", icon = icon("bed"),
                 menuSubItem("Accommodation in Melbourne", tabName = "Accommodation")
        ),
        
        
        # Live music menu
        menuItem("Live music", tabName = "live_music", icon = icon("music"),
                 
                 menuSubItem("Live music in Melbourne", tabName = "live_music_tab")
        ),
        fluidRow(
          column(12,
                 ...
                 HTML(paste('<p style="font-weight: bold;">  Current Day: ', current_day, '</p>'))
          )
        )
      ...
      tabItems(
        tabItem(
          ...
          box(title = "Come and Say G’day", "Explore the beauty and wonders of Australia with our comprehensive tourism dashboard. Australia is a vast and 
                    ...
                    about this remarkable destination, our dashboard provides you with key insights and information to make your Australian adventure unforgettable.",width= "300px"),
          HTML('<div style="text-align:center;">
                   <img src="https://www.travelmole.com/wp-content/uploads/2021/10/Tourism_Australia_logo_wordmark_horizontal.png" width="1000" height="500">
                </div>'),
          HTML('<div style="text-align:center;">
        ...
        tabItem(
          ...
          fluidRow(
            splitLayout(
              ...
              tableauPublicViz(
                id='tableauViz', 
                url='https://public.tableau.com/views/Book1_16975116479590/Sheet5?:language=en-US&:display_count=n&:origin=viz_share_link',
                height="600px",
                width = "120%"
              ),
              tableauPublicViz(
                id='tableauViz', 
                url='https://public.tableau.com/views/Book2_16978600520920/Sheet4?:language=en-US&:display_count=n&:origin=viz_share_link',
                height="600px",
                width = "120%"
              )
        ...
        tabItem("Tourism_Industries",
                ...
                sidebarLayout(
                  sidebarPanel(
                    HTML('<h3 style="font-weight: bold;">Melbourne Entertainment</h3>'),
                    selectInput(
                      'industry',
                      label = HTML('<span style="font-weight: bold; font-style: italic;">Tourism Industry</span>'),
                    ...
                    selectInput(
                      'suburbs',
                      label = HTML('<span style="font-weight: bold; font-style: italic;">Suburbs</span>'),
        ...
        tabItem("on-street",
                mainPanel(
                  ...
                  h3(class = "red_reminder","Note: Please input the value of Search Content below!!!"),
                  fluidRow(
                    column(
                      width = 5,
                      textAreaInput("searchTXT1",label = HTML('<i class="fas fa-search"></i> Search Content'))
                    ),
                    column(
                      width = 3,
                      numericInput(label = HTML('<i class="fas fa-search"></i> Search Radius (meter)'),"R1",value = 3000, min = 200, max = 5000)
                    ),
                    column(
                      width = 3,
                      numericInput("quantity1",label = HTML('<i class="fas fa-search"></i> Number of Results'), value = 10, min = 1, max = 20)
                    )
                  ),
        ...
        tabItem("service",
                mainPanel(
                  ...
                  fluidRow(
                    column(
                      width = 5,
                      textAreaInput("searchTXT2",label = HTML('<i class="fas fa-search"></i> Search Content'))
                    ),
                    column(
                      width = 3,
                      numericInput(label = HTML('<i class="fas fa-search"></i> Search Radius (meter)'),"R2",value = 3000, min = 200, max = 5000)
                    ),
                    column(
                      width = 3,
                      numericInput("quantity2",label = HTML('<i class="fas fa-search"></i> Number of Results'), value = 10, min = 1, max = 20)
                    )
                  ),
                  fluidRow(
                    column(
                      width = 4,
                      selectInput('Category',label = HTML('<i class="fas fa-list"></i> Category'),choices = c('Helpful phone number','Legal / Financial Advice','Drug and Alcohol',
        ...
        #Live music layout
        tabItem("live_music_tab",
                header=setUpTableauInShiny(),
                fluidRow(
                  #HTML('<h3 style="font-weight: bold;">Melbourne Live Music</h3>'),
                  selectInput(
                    'EventSpace',
                    #label = HTML('<span style="font-weight: bold; font-style: italic;">Event Venue</span>'),
                    label = 'Event Venue',
                    choices = c(sort(unique(music_venues$space_type))),
                    selected = 'Hotel'
                  ),
                  ...
                  selectInput(
                    ...
                    #label = HTML('<span style="font-weight: bold; font-style: italic;">Suburbs</span>'),
                    label = 'Suburbs',
                    choices = c(sort(unique(music_venues$suburb_name))),
                    selected = 'Melbourne (CBD)'
                  ),
                ),
                fluidRow(
                  leafletOutput('locations',width="140%",height="450px")
                ),
                br(),
                fluidRow(
                  splitLayout(
                    ...
                    tableauPublicViz(
                      id = 'tableauViz_live_music', 
                      url = 'https://public.tableau.com/views/live_music_venues/Sheet4?:language=en-US&publish=yes&:display_count=n&:origin=viz_share_link',
                      height=700,
                      width="200%"
                    )
                  ))
...
server <- function(input, output, session) {
  ...
  plot_info = 
    ...
    today_data[c("Name","Longitude","Latitude","Category.1",
                 "Suburb","Tram.routes","Cost"
                 ,"Popup","eventIcon")]
  ...
  plot_bike = 
    ...
    bike_data[c("name","capacity","lat","lon",
                "SplitLocation1","SplitLocation2","SplitLocation3")]
  #print(bike_data)
  ...
  output$plot_bike <- renderLeaflet({
    ...
    suburb <- input$tableauVizBike_mark_selection_changed$SuburbforBike
  ...
  observeEvent(input$industry, {
    ...
    runjs(sprintf('let viz = document.getElementById("tableauViz_entertainment");
    ...
    m <- leaflet(data = filtered_data()) %>%
      ...
      addAwesomeMarkers(lng=~longitude, lat=~latitude,
                        ...
                        icon=~awesomeIcons(
                          iconColor = "#ffffff",
                          markerColor = 'orange'),
                        popup=~popup_content(room_type, name, price))
    m
  })
  ...
  # update the area selection based on the event space
  observeEvent(input$EventSpace, {
    
    updated_choices <- unique(music_venues$suburb_name[music_venues$space_type == input$EventSpace])
    
    # update selections
    updateSelectInput(session, "suburb_names", choices = updated_choices)
    
    # Filter Tableau viz based on event type
    space_type <- input$EventSpace
    runjs(sprintf('let viz = document.getElementById("tableauViz_live_music");
        let sheet = viz.workbook.activeSheet;
        sheet.applyFilterAsync("Space Type", ["%s"], FilterUpdateType.Replace);', space_type))
  ...
  getDataALL <- reactive({
    filter(music_venues, 
           space_type == input$EventSpace,
           suburb_name == input$suburb_names)})
  
  iconSet <- awesomeIconList(
    hotel = makeAwesomeIcon(
      icon = 'bed',
      library = 'fa',
      iconColor = 'rgb(179, 107, 0)',
      markerColor = 'orange'
    ),
    bar = makeAwesomeIcon(
      icon = 'beer',
      library = 'fa',
      iconColor = 'rgb(204, 122, 0)',
      markerColor = 'orange'
    ),
    multi_use = makeAwesomeIcon(
      icon = 'bank',
      library = 'fa',
      iconColor = 'rgb(230, 138, 0)',
      markerColor = 'orange'
    ),
    nightclub = makeAwesomeIcon(
      icon = 'music',
      library = 'fa',
      iconColor = 'rgb(255, 153, 0)',
      markerColor = 'orange'
    ),
    performance_space = makeAwesomeIcon(
      icon = 'building',
      library = 'fa',
      iconColor = 'rgb(255, 163, 26)',
      markerColor = 'orange'
    ),
    
    theatre = makeAwesomeIcon(
      icon = 'sign-language',
      library = 'fa',
      iconColor = 'rgb(255, 173, 51)',
      markerColor = 'orange'
    ),
  ...
  output$locations <- renderLeaflet({
    leaflet(getDataALL()) %>%
      ...
      addAwesomeMarkers(lng = ~lon, lat = ~lat, popup = ~paste("Venue name:",venue_name,
                                                               "<br>","Venue address:", venue_address,
                                                               "<br>","Venue type:",space_type,
                                                               "<br>", "Website: ",
                                                               '<a href=', website, '>',
                                                               "Event link", '</a>'),
                        icon=~awesomeIcons(library='fa',
